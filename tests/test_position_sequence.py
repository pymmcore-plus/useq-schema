...
def test_channel_only_in_position_sub_sequence():
    ...
    assert [
        (i.global_index, i.index, (i.channel.config if i.channel else None), i.exposure)
        for i in mda.iter_events()
    ] == [(0, {"p": 0}, None, None), (1, {"p": 1, "c": 0}, "488", 100.0)]
...
def test_channel_in_main_and_position_sub_sequence():
    ...
    assert [
        (i.global_index, i.index, (i.channel.config if i.channel else None), i.exposure)
        for i in mda.iter_events()
    ] == [(0, {"p": 0, "c": 0}, "Cy5", 50.0), (1, {"p": 1, "c": 0}, "488", 100.0)]
...
def test_grid_relative_only_in_position_sub_sequence():
    ...
    assert [
        (i.global_index, i.index, i.pos_name, i.x_pos, i.y_pos)
        for i in mda.iter_events()
    ] == [
        (0, {"p": 0}, None, 0.0, 0.0),
...
def test_z_relative_with_multi_stage_positions():
    ...
    assert [
        (i.global_index, i.index, i.x_pos, i.y_pos, i.z_pos) for i in mda.iter_events()
    ] == [
...
def test_z_relative_only_in_position_sub_sequence():
    ...
    assert [
        (i.global_index, i.index, i.pos_name, i.z_pos) for i in mda.iter_events()
    ] == [
        (0, {"p": 0}, None, 0.0),
        (1, {"p": 1, "z": 0}, "test", 9.0),
        (2, {"p": 1, "z": 1}, "test", 10.0),
...
def test_order():
    ...
    assert [
        (i.global_index, i.index, i.z_pos, i.channel.config) for i in mda.iter_events()
    ] == [
