...
NAME = "name"
EMPTY: dict = {}
CH_FITC = Channel(config=FITC, exposure=100)
CH_CY5 = Channel(config=CY5, exposure=50)
Z_RANGE2 = ZRangeAround(range=2, step=1)
Z_RANGE3 = ZRangeAround(range=3, step=1)
Z_28_30 = ZTopBottom(bottom=28, top=30, step=1)
Z_58_60 = ZTopBottom(bottom=58, top=60, step=1)
GRID_2x2 = GridRowsColumns(rows=2, columns=2)
GRID_1100 = GridFromEdges(top=1, bottom=-1, left=0, right=0)
GRID_2100 = GridFromEdges(top=2, bottom=-1, left=0, right=0)
SEQ_1_CH = MDASequence(channels=[CH_FITC])
TLOOP2 = TIntervalLoops(interval=1, loops=2)
TLOOP3 = TIntervalLoops(interval=1, loops=3)
TLOOP5 = TIntervalLoops(interval=1, loops=5)
...
def expect_mda(mda: MDASequence, **expectations: Sequence[Any]) -> None:
    ...
    for attr_name, actual_value in results.items():
        assert actual_value == expectations[attr_name], f"{attr_name!r} mismatch"
...
def test_channel_only_in_position_sub_sequence() -> None:
    ...
    seq = MDASequence(
        stage_positions=[EMPTY, MDASequence(value=Position(), channels=[CH_FITC])],
    )
...
def test_grid_relative_only_in_position_sub_sequence() -> None:
    ...
    mda = MDASequence(
        stage_positions=[
            Position(x=0, y=0),
            MDASequence(value=Position(x=10, y=10), grid_plan=GRID_2x2),
        ],
    )
...
def test_grid_absolute_only_in_position_sub_sequence() -> None:
    mda = MDASequence(
        stage_positions=[
            Position(x=0, y=0),
            MDASequence(value=Position(), grid_plan=GRID_1100),
        ],
    )
...
def test_grid_relative_in_main_and_position_sub_sequence() -> None:
    mda = MDASequence(
        stage_positions=[
            Position(x=0, y=0),
            MDASequence(value=Position(name=NAME, x=10, y=10), grid_plan=GRID_2x2),
        ],
        grid_plan=GRID_2x2,
    )
    expect_mda(
...
def test_grid_absolute_in_main_and_position_sub_sequence() -> None:
    mda = MDASequence(
        stage_positions=[
            EMPTY,
            MDASequence(value=Position(name=NAME), grid_plan=GRID_2100),
        ],
        grid_plan=GRID_1100,
    )
    expect_mda(
...
def test_grid_relative_in_main_and_grid_absolute_in_position_sub_sequence() -> None:
    mda = MDASequence(
        stage_positions=[
            Position(x=0, y=0),
            Position(name=NAME, sequence={"grid_plan": GRID_1100}),
        ],
        grid_plan=GRID_2x2,
    )
    expect_mda(
...
def test_z_absolute_with_multi_stage_positions() -> None:
    expect_mda(
        MDASequence(stage_positions=[Position(x=0, y=0), (10, 20)], z_plan=Z_58_60),
        index=genindex({"p": 2, "z": 3}),
        x_pos=[0.0, 0.0, 0.0, 10.0, 10.0, 10.0],
        y_pos=[0.0, 0.0, 0.0, 20.0, 20.0, 20.0],
...
def test_z_relative_only_in_position_sub_sequence() -> None:
    mda = MDASequence(
        stage_positions=[
            Position(z=0),
            Position(name=NAME, z=10, sequence={"z_plan": Z_RANGE2}),
        ],
    )
...
def test_z_absolute_only_in_position_sub_sequence() -> None:
    mda = MDASequence(
        stage_positions=[
            Position(z=0),
            Position(name=NAME, sequence={"z_plan": Z_58_60}),
        ],
    )
...
def test_z_relative_in_main_and_position_sub_sequence() -> None:
    mda = MDASequence(
        stage_positions=[
            Position(z=0),
            Position(name=NAME, z=10, sequence={"z_plan": Z_RANGE3}),
        ],
        z_plan=Z_RANGE2,
    )
...
def test_z_absolute_in_main_and_position_sub_sequence() -> None:
    mda = MDASequence(
        stage_positions=[
            EMPTY,
            Position(name=NAME, sequence={"z_plan": Z_28_30}),
        ],
        z_plan=Z_58_60,
    )
    expect_mda(
...
def test_order() -> None:
    sub_pos = Position(
        z=50,
        sequence=MDASequence(channels=[CH_FITC, Channel(config=CY3, exposure=200)]),
    )
    mda = MDASequence(
        stage_positions=[Position(z=0), sub_pos],
...
def test_channels_and_pos_grid_plan() -> None:
    ...
    sub_seq = MDASequence(grid_plan=GridRowsColumns(rows=2, columns=1))
    mda = MDASequence(
        channels=[CH_CY5, CH_FITC],
        stage_positions=[Position(x=0, y=0, sequence=sub_seq)],
    )
...
def test_channels_and_pos_z_plan() -> None:
    ...
    mda = MDASequence(
        channels=[CH_CY5, CH_FITC],
        stage_positions=[Position(x=0, y=0, z=0, sequence={"z_plan": Z_RANGE2})],
    )
    expect_mda(
        mda,
...
def test_channels_and_pos_time_plan() -> None:
    ...
    mda = MDASequence(
        axis_order="tpgcz",
        channels=[CH_CY5, CH_FITC],
        stage_positions=[Position(x=0, y=0, sequence={"time_plan": [TLOOP3]})],
    )
    expect_mda(
        mda,
...
def test_channels_and_pos_z_grid_and_time_plan() -> None:
    ...
    sub_seq = MDASequence(grid_plan=GRID_2x2, z_plan=Z_RANGE2, time_plan=[TLOOP2])
    mda = MDASequence(
        channels=[CH_CY5, CH_FITC],
        stage_positions=[Position(x=0, y=0, sequence=sub_seq)],
    )
