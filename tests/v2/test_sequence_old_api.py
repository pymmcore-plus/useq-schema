"""This test is copied from v1 test_sequence.

To see if we have API parity, and to make sure the old API still works.
"""
...
import useq.v2._grid as v2_grid
...
def RelativePosition(**k: Any) -> Position:
    """Create a RelativePosition with default values."""
    return Position(**k, is_relative=True)
...
@pytest.mark.filterwarnings("ignore:num_positions")
@pytest.mark.parametrize("zplan, zexpectation", z_inputs)
def test_z_plan(zplan: v2.ZPlan, zexpectation: Sequence[float]) -> None:
    z_plan = MDASequence(z_plan=zplan).z_plan
    assert isinstance(z_plan, v2.ZPlan)
    # zpos_expectation = [RelativePosition(z=z) for z in zexpectation]
...
def test_g_plan(gridplan: Any, gridexpectation: Sequence[Any]) -> None:
    ...
    assert isinstance(g_plan, v2_grid._GridPlan)
...
@pytest.mark.filterwarnings("ignore:num_timepoints")
...
def test_keep_shutter_open() -> None:
    ...
    subseq = MDASequence(grid_plan=GridRowsColumns(rows=2, columns=2))
    mda5 = MDASequence(
        axis_order="pgc",
        channels=["DAPI", "FITC"],
